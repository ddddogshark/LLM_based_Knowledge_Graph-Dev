...
async def main():
    ...
    courses = [
        {
            "course_name": "DSAA_2088_Mathematics for Data Science",
            "data_path": "C:\\1DevProject\\LLM_based_Knowledge_Graph\\data\\DSAA_2088_Mathematics for Data Science"
        }
    ]

    for course in courses:
        print(f"--- Running pipeline for course: {course['course_name']} ---")
        initial_context = {
            "course_name": course["course_name"],
            "data_path": course["data_path"]
        }
        final_context = await orchestrator.run_pipeline(initial_context)
        
        print(f"\n--- Final Context for {course['course_name']} ---")
        ...
        print(f"\n--- Pipeline Status for {course['course_name']} ---")
        print(orchestrator.get_pipeline_status())
        print(f"--- Finished pipeline for course: {course['course_name']} ---\n")
