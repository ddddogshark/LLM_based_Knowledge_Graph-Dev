...
1.  **`MultimodalParserAgent`**:
    ...
    *   **算法**:
        ...
        5.  **生成课程Markdown文件**: 对于每个成功解析的课程，将该课程下所有讲座的Markdown内容和图片路径聚合成一个单独的Markdown文件。该文件以课程名称命名（例如 `课程名.md`），并保存在项目的根目录下。这为每个课程提供了一个完整的、可供查阅的文本和图片资源概览。
...
2.  **`ContentUnderstandingAgent`**:
    ...
    *   **逻辑**: 对从本地文件中读取到的所有文本内容进行整合和智能切分，然后利用LLM从这些文本块中提取结构化的“知识点草稿”，确保在处理长文本时信息的完整性。
    ...
    *   **算法**:
        ...
        1.  从上下文中获取所有来源的文本内容（本地文件、网络爬虫等），并将其合并成一个单一的、巨大的字符串。



        2.  **文本切分**: 使用一个简单的递归文本切分函数 (`src/utils/text_splitter.py`) 对合并后的长文本进行切分。这个函数会尝试使用多种分隔符（如段落、换行符、空格）来寻找自然的文本断点，将长文本切分成大小合适的块。



        3.  **并行处理**: 将切分后的文本块并发地（Asynchronously）发送给LLM。



        4.  为每个文本块构建一个Prompt，要求LLM将其内容转换成包含“标题”、“解释”和“关键词”的知识点。



        5.  收集所有LLM返回的结果，并将其合并成一个完整的知识点草稿列表。



        6.  将知识点草稿列表存入共享的上下文，并同时将其格式化为一个Markdown文件 (`knowledge_point_drafts.md`) 保存到本地，以便查阅。
